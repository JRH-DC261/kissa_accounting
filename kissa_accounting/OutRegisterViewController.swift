...
class OutRegisterViewController: UIViewController{
    ...
    @IBAction func add(_ sender: Any) {
        ...
        totalAmount.text = "\(SAmount*200 + DAmount*100)"
    }
    ...
    @IBAction func check(_ sender: Any) {
        ...
        let okAction = UIAlertAction(title: "OK", style: UIAlertAction.Style.default){ (action: UIAlertAction) in
            self.DBRef.child("table/status").child(self.tableNumber!).observeSingleEvent(of: .value, with: { (snapshot) in
                ...
                if Int(self.status!)! != 0{
                    ...
                    let OKButton = UIAlertAction(title: "OK", style: UIAlertAction.Style.default){ (action: UIAlertAction) in
                        ...
                        self.DBRef.child("table/order").child(self.tableNumber!).setValue(["S1Amount":self.S1Amount!, "S2Amount":self.S2Amount!, "D1Amount":self.D1Amount!, "D2Amount":self.D2Amount!, "D3Amount":self.D3Amount!, "D4Amount":self.D4Amount!, "D11Amount":self.D11Amount!, "D12Amount":self.D12Amount!, "D13Amount":self.D13Amount!, "D14Amount":self.D14Amount!, "D15Amount":self.D15Amount!, "totalSAmount":self.totalSAmount!,  "totalDAmount":self.totalDAmount!, "time":ServerValue.timestamp()])
                        self.DBRef.child("table/status").child(self.tableNumber!).setValue(1)
                        
                        //オーダーキーの設定
                        let key = self.DBRef.child("table/orderOrder").childByAutoId().key;
                        self.DBRef.child("table/orderOrder").child(key).setValue(self.tableNumber!)
                        self.DBRef.child("table/orderKey").child(self.tableNumber!).setValue(key)
                        //データセット
                        self.DBRef.child("inData").child(key).setValue(["time":ServerValue.timestamp(), "table":self.tableNumber!, "S1":self.S1Amount!, "S2":self.S2Amount!, "D1":self.D1Amount!, "D2":self.D2Amount!,"D3":self.D3Amount!, "D4":self.D4Amount!, "D11":self.D11Amount!, "D12":self.D12Amount!, "D13":self.D13Amount!,"D14":self.D14Amount!, "D15":self.D15Amount!])
                        ...
                        defaultPlace.observeSingleEvent(of: .value, with: { (snapshot) in
                            ...
                            guard let client = DropboxClientsManager.authorizedClient else {
                                return}
                            let fileData = "\(self.tableNumber!),/\(hogeKey!)/,\(self.SAmount),\(self.DAmount),\(self.tenderAmount.text!)".data(using: String.Encoding.utf8, allowLossyConversion: false)!
                            _ = client.files.deleteV2(path: "/kissa2019.system/kissa_accounting_reciept/Data/receiptOut.csv")
                            ...
                            _ = client.files.upload(path:"/kissa2019.system/kissa_accounting_reciept/Data/receiptOut.csv", input: fileData)
                                .response { response, error in
                                    if let response = response {
                                        print(response)
                                    }else if let error = error {
    ...
    @objc func amountload(_ sender: Timer) {
        ...
        let defaultPlaceD1 = DBRef.child("table/order").child(tableNumber!).child("D1Amount")
        ...
        let defaultPlaceD2 = DBRef.child("table/order").child(tableNumber!).child("D2Amount")
        ...
        let defaultPlaceD3 = DBRef.child("table/order").child(tableNumber!).child("D3Amount")
        ...
        let defaultPlaceD4 = DBRef.child("table/order").child(tableNumber!).child("D4Amount")
        ...
        /*let defaultPlaceD5 = DBRef.child("table/order").child(tableNumber!).child("D5Amount")
         ...
         let defaultPlaceD6 = DBRef.child("table/order").child(tableNumber!).child("D6Amount")
         ...
         let defaultPlaceD7 = DBRef.child("table/order").child(tableNumber!).child("D7Amount")
         ...
         let defaultPlaceD8 = DBRef.child("table/order").child(tableNumber!).child("D8Amount")
         ...
         let defaultPlaceD9 = DBRef.child("table/order").child(tableNumber!).child("D9Amount")
         ...
         let defaultPlaceD10 = DBRef.child("table/order").child(tableNumber!).child("D10Amount")
        ...
        let defaultPlaceD11 = DBRef.child("table/order").child(tableNumber!).child("D11Amount")
        defaultPlaceD11.observeSingleEvent(of: .value, with: { (snapshot) in
        ...
        let defaultPlaceD12 = DBRef.child("table/order").child(tableNumber!).child("D12Amount")
        defaultPlaceD12.observeSingleEvent(of: .value, with: { (snapshot) in
        ...
        let defaultPlaceD13 = DBRef.child("table/order").child(tableNumber!).child("D13Amount")
        defaultPlaceD13.observeSingleEvent(of: .value, with: { (snapshot) in
        ...
        let defaultPlaceD14 = DBRef.child("table/order").child(tableNumber!).child("D14Amount")
        defaultPlaceD14.observeSingleEvent(of: .value, with: { (snapshot) in
        ...
        let defaultPlaceD15 = DBRef.child("table/order").child(tableNumber!).child("D15Amount")
        defaultPlaceD15.observeSingleEvent(of: .value, with: { (snapshot) in
